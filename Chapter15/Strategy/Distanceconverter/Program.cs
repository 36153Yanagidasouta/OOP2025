...
namespace Distanceconverter {
    internal class Program {
        static void Main(string[] args) {
            ...
            var from = GetConverter("変換元の単位を入れてください");
            var to = GetConverter("変換先の単位を入れてください");
            var distace = GetDistance(from);

            var converter = new DistanceConverter(from, to);
            var result = converter.Convert(distace);
            var text = $"{distace}{from.UnitName}は、{result:0.000}{to.UnitName}です\n";
        ...
        static ConverterBase GetConverter(string msg) {
            ...
            do {
                ...
                if (unit != null)
                    converter = ConverterFactory.GetConverter(unit);
